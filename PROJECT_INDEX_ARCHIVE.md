# SEC Filing Research Agent — Project Index Archive

> Overflow from `PROJECT_INDEX.md`'s `## Recent` section, moved here in
> batches whenever `Recent` exceeds its 50-entry cap (trimmed back to
> 40). **Never read this file in full** — search it with the Grep tool
> (a ticker, module/file name, tool name, or failure-mode phrase) when a
> task might touch history older than what's visible in
> `PROJECT_INDEX.md`'s `Recent` section, then open a specific linked
> file only on a real hit. Same one-line-per-file, reverse-chronological
> format as `Recent`; `TEMPLATE.md` excluded from each `docs/` directory
> the same way.
