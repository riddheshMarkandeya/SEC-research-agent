...
## Critical Bug Fix Report: Gemini Client Garbage Collection

### Bug Description
Task 7 live verification run found a critical bug: the Gemini backend failed on every multi-turn conversation with `RuntimeError: Cannot send a request, as the client has been closed.` This only appeared during live testing (Task 7), not in unit tests, because the bug involves object lifetime and garbage collection.

**Root cause:** `_gemini_start()` created the API client as a local variable and never returned it:
```python
def _gemini_start(...):
    client = genai.Client(api_key=GEMINI_API_KEY)  # local variable
    ...
    return chat, ...  # client is dropped; goes out of scope
```

When the function returns, `client` goes out of scope and gets garbage-collected. The `google-genai` SDK's `ApiClient` has a `__del__` that closes the underlying httpx transport when collected. So the very next call (`_gemini_send()`) would fail because the transport was already closed.

This bug caused **100% failure on every eval question** (27 questions + 3 sanity checks all failed at turn 2).

### Fix Applied
Implemented lazy client initialization with module-level caching:
...
**Updated `_gemini_start()`:**
```python
def _gemini_start(question: str, system_prompt: str, tool_schemas: list[dict]) -> tuple[object, ModelTurn]:
    client = _get_gemini_client()  # Get cached instance
    # ... rest of function unchanged
```

**Benefits of this approach:**
- Client stays alive across all turns (not garbage-collected)
- Lazy initialization — client only created when `--backend gemini` is actually used
- Ollama users never need GEMINI_API_KEY set (preserves Task 4's design intent)
- API key check still only happens when Gemini backend is selected, not at import time
...
```
Command: .venv\Scripts\python.exe -m pytest tests/test_llm_backends.py -v
Output: 6 passed (3 Ollama + 3 Gemini normalization tests)
```

**Full suite:**
```
Command: .venv\Scripts\python.exe -m pytest
Output: 226 passed, 2 warnings in 12.31s
```

**Direct verification — client caching works:**
```python
# Verify same instance returned on repeated calls:
client1 = _get_gemini_client()
client2 = _get_gemini_client()
assert client1 is client2  # ✓ PASS
```

### Impact
This fix enables the Gemini backend to work in multi-turn conversations. Without it, every conversation fails at turn 2. This was the blocker preventing Task 7's live verification from completing.

### Commit
```
356a9b5 Fix: Prevent Gemini client garbage collection on multi-turn conversations
```

Changes: 1 file, 21 insertions, 6 deletions (added client cache + helper function, simplified `_gemini_start()`)
