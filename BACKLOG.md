...
- [ ] **[bug, High, Substantial]** `MAX_TOOL_ITERATIONS=6` leaves exactly zero slack for a legitimate final answer: `calls_made` starts at 1, increments once per round trip, and dispatch is gated on `calls_made < 6` -- so a question needing all 5 dispatch turns to gather real data has NO turn left to call `submit_answer` on, even when every call succeeded and the agent already holds everything it needs. Confirmed live: `nvda-rd-expense-q4fy26-refusal` and `pltr-inventory-turnover-fy2025-refusal` are both correct-REFUSAL questions (the data genuinely isn't available) that made 5 fully successful calls, hit zero errors, and timed out with nothing left to submit the refusal itself -- both have independently submitted that exact correct refusal successfully in other live runs when the turns happened to fall right, confirming this is a budget-shape problem, not a capability gap. Deliberately scoped OUT of the 2026-09-14 fix: any remedy (a final-turn forced-submit safety net, or exempting failed/rejected tool calls from the budget) is a tool-calling-LOOP change, touching `calls_made`/`forced_submit_attempted`/`retried_for_citations`/`pre_retry_submit_args` together, and carries its own real wrong-answer risk -- a forced final submit on a ranking run that's missing CRM (see the fiscal-year-label bug above) would confidently name the wrong company, the exact failure mode this project already rejected raising `MAX_TOOL_ITERATIONS` outright to avoid. Needs its own plan (design a safety net that can't produce a confidently-wrong grounded answer), not a quick patch.
- [ ] **[bug, Med, Standard]** On a purely qualitative "what risks does X describe" question (no real number to report), the model sometimes submits `claims` entries with a small placeholder integer VALUE (1, 2, 3...) per bullet point, quoting long qualitative prose that doesn't literally contain that digit as a number -- `verify_claims` correctly refuses these (`"[N] claims N (raw) but that value doesn't appear in the quoted text"`), but the underlying answer was otherwise fine. Confirmed live on 3 questions in the 2026-09-14 baseline (`aapl-ai-risk`, `crm-ai-risk`, `pltr-government-contract-risk`) via the actual submitted `claims` array in `trace_logs/traces.jsonl` -- not guessed. Root cause is plausibly rule 9's "for EVERY number in `answer_text`, add a matching entry to `claims`" being over-applied when there's no real number at all, combined with the `claims` schema's `value` field having no way to express "this citation supports a qualitative fact, not a numeric one." The identical `"claims N (raw)"` shape was ALSO present in the 2026-09-13 baseline on different questions (`nvda-supply-chain-risk`, `pltr-government-contract-risk`) -- a real, recurring, previously-untracked bug class, not a one-off. Not fixed this session (out of scope for the turn-waste fix); needs either a rule-9 clarification (qualitative facts need a citation marker in `answer_text` but not a `claims` entry) or a schema change (an explicit `qualitative: true` claim kind that skips numeric grounding).
